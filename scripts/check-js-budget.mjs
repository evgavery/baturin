// JS-бюджет на страницу (ТЗ §12: ≤ 50 КБ gzip). Обходит dist/**/*.html рекурсивно, для каждой
// страницы суммирует честный eager-вес JS:
//
//   1. внешние `<script src="...">` — читает файл-ассет из dist и гзипует (zlib.gzipSync).
//      Сюда попадают все переиспользуемые чанки (Base/app.ts — на всех страницах, CtaBlock/
//      short-form, QualifyForm/qualify-form, led-ekrany/led-calc — там, где подключены):
//      Vite всегда выносит их в отдельный файл, раз на них есть импорт из общих модулей
//      (goals.ts, form-utils.ts, form-submit.ts, SITE и т.п.).
//   2. инлайновые `<script type="module">` с непустым телом — Vite инлайнит прямо в HTML
//      «листовые» чанки без единого общего импорта (в проекте это только hero-slider.ts на
//      главной: он самодостаточен, ни от чего общего не зависит). Это ТОЖЕ реальный
//      исполняемый JS страницы, и hero-slider.ts осознанно eager (Task 13 brief, Step 2) —
//      бюджет обязан его ловить, иначе страница «главная» была бы недосчитана.
//
// Явно НЕ считаем:
//   - `<script type="application/ld+json">` — структурные данные, не JS;
//   - инлайн-сниппет Яндекс.Метрики (`is:inline`, без type="module") — санкционированное
//     Global Constraints единственное внешнее стороннее подключение, вне бюджета своего JS;
//   - квиз-чанк (динамический import по клику, Task 10/11) — он в HTML не попадает вообще
//     ни как `<script src>`, ни как `<link rel="modulepreload">` (проверено вручную), поэтому
//     естественным образом не считается — бюджет честно отражает только eager-вес страницы.
//
